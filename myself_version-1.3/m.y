...
triangle:SINE '(' expression ')' ';' 		{ double a = sin(($3*pi)/180); printf("in sin block %s = %lf\n",$1,a); }
		|COSINE '(' expression ')' ';' 		{ double a = cos(($3*pi)/180); printf("in cos block %s = %lf\n",$1,a); }
		|TANGENT '('expression ')' ';' 		{ double a = tan(($3*pi)/180); printf("in tan block %s = %lf\n",$1,a); }
		|LOG '('expression ')' ';' 			{ double m = log($3); printf("in log block %s =%lf\n",$1,m); }
		;
...
expression: NUM { $$ = $1;}
		| VAR { if(!declared[$1]){printf("%c variable not declared\n",$1+'a');}
							else{$$ = sym[$1];} }
...
equation: expression '=' expression ';' {
		...
		|expression EQ triangle ';' {printf ("value \n"); }
			;
...
print : SHOW '(' expression ')' ';' {printf("value of %c = ",$3 + 'a');}
;			
%%
/*
int checkState(int m){
