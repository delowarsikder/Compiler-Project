...
%token  PRESS AUTO CHECK CHECK_END SHOW ADD SUB MULT DIV GT GTE LT LTE MOD DEQ MATCH EQ NEQ FACT AND OR  NOT  ENERGY LOOP HEADER_NAME HEADER START END MAIN FROM TO SPLIT REAL COMPLEX

%token <double_val>SINE COSINE TANGENT LOG
%token <number> NUM
%token <string>BUTTON
...
triangle:SINE '(' expression ')' ';' 		{ float a = sin($3); printf("in sin block %s = %f\n",$1,a); }
		|COSINE '(' expression ')' ';' 		{ printf("in cos block %s\n",$1); }
		|TANGENT '('expression ')' ';' { printf("in tan block %s\n",$1); }
		|LOG '('expression ')' ';' { printf("in log block %s\n",$1); }
		;
